...
- 若输入中提供了「当前完整 YAML」，且存在可安全表达的优化，请输出完整 optimizedYaml；若没有明确改动但 YAML 本身可作为最终状态，也可输出等价的完整 YAML。
- 若只收到截断样本、无法保证与现网语义等价、或会丢字段，将 optimizedYaml 置为空，并在 suggestions 中说明人工步骤。
- 不要输出 JSON 以外的字符。`;
