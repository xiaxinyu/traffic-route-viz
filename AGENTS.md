...
- 资源统计（本地目录树）：`web/src/features/resource-stats/`（`hooks/useLocalFolderScan.ts`、`ResourceStatsHeader.tsx`、`ResourceStatsLeftPanel.tsx`、`ResourceStatsFileInspector.tsx`、`ResourceStatsCodePanel.tsx` 等）；领域：`web/src/domain/buildFileTree.ts`、`fileTreeQueries.ts`、`fileTreeExpansion.ts`、`formatByteSize.ts`、`fileSummary.ts`；页面壳 `web/src/app/ResourceStatsHome.tsx` 仅 re-export。
